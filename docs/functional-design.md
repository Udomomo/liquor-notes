...
| フィールド | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| name | string | No | お酒の名前（部分一致）。省略または空文字で条件なし |
| rate.minimum | string | No | 評価の最小値（数値文字列）。省略または空文字で条件なし。数値にパースできない場合は400 |

- `name` と `rate.minimum` の両方を指定した場合は AND 検索
- 検索条件がない場合は自分が投稿した全件を取得
...
**エラー（400）:** `rate.minimum` が数値にパースできない場合
```json
{ "error": "rate.minimum must be a number" }
```
